...
def plot_trend_over_time(embs: np.array, times: list, google_trends_df, t_prob, anomalies):
    ...
    overlay.opts({'Curve': {'plot': dict(width=900, height=500)}})
...
if __name__ == '__main__':
    ...
    t_emb_mean, t_emb_weighted_mean, t_prob = get_embeddings_by_paths_average(data_df.sample(10_000), model_path, dataset_name, walk_len=32)
    ...
    trend_plot = plot_trend_over_time(embs=temporal_embeddings, times= t_prob.keys(), google_trends_df=google_trends_df,
                                      t_prob=t_prob.values(), anomalies=anoms)